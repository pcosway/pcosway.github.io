...
use constant CACHE_TIME => 60*60*24*15; #3600; # how long to cache browse sessions - 15 days, if that is what it takes!
...
#$log->info("query source: ",Data::Dumper::Dumper($request));
#$log->info("req ",$request->getParam('_clientid'));
#$log->info("req2 ",$request->{'_clientid'});
my $reqsource = $request->{'_clientid'};
...
#$log->info('req ',$request->{'_clientid'};
#	my $reqsource = $request->{'_clientid'};
	...
	# PCosway - moved to the start, end is too late!
...
$log->info('not using cache ',$itemId);
...
#	my $forceTitle = $params->{'forceTitle'};
	my $client     = $request->client();
	my $window;
#$log->info("query", Data::Dumper::Dumper($query));
#$log->info("client id is ",$request->getParam('_clientid'));
	my $reqsource = $request->{'_clientid'};
#$log->info("client id is ",$reqsource);
#$log->info("2client id is ",$request->{'_clientid'});
#$log->info("3client id is ",$request->getParam('clientid'));
...
$sid .= $reqsource;
#my $newsid = $sid . $reqsource;
#$log->info('new sid ',$newsid);
